...
async function api(m,p,b,isF){
  ...
  const r=await fetch(`/docflow/api${p}`,o);
...
async function doLogin(){
  ...
  try{
    const r=await fetch('/docflow/api/auth/login',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email,password:pass})});
...
async function doForgot(){
  ...
  if(!email)return;
  try{
    await fetch('/docflow/api/auth/forgot-password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({email})});
...
function showApp(){
  ...
  // Cargar versión
  fetch('/docflow/api/version').then(r=>r.json()).then(d=>{
...
async function descargarBackup(tipo='completo'){
  ...
  try{
    await fetch('/docflow/api/backup?action=generate&tipo=config',{headers:{Authorization:`Bearer ${token}`}});
  }catch(e){
  ...
  window.cancelarBackupGen=async function(){
      ...
      try{await fetch('/docflow/api/backup/cancelar',{method:'POST',headers:{Authorization:'Bearer '+token}})}catch(_){}
      closeM();
      btn.disabled=false;
  ...
  try{
    ...
    const url=tipo==='config'?'/docflow/api/backup?action=generate&tipo=config&_='+Date.now():'/docflow/api/backup?action=generate&tipo=completo&_='+Date.now();
    ...
    pollInterval=setInterval(async()=>{
      try{
        const p=await fetch('/docflow/api/backup/progreso',{headers:{Authorization:`Bearer ${token}`}}).then(r=>r.json());
...
async function descargarBackupLocal(n){
  try{
    const token=localStorage.getItem('vd_t');
    const resp=await fetch('/docflow/api/backup/descargar/'+encodeURIComponent(n),{headers:{Authorization:`Bearer ${token}`}});
...
async function restaurarBackupLocal(n){
  ...
  try{
    const token=localStorage.getItem('vd_t');
    const resp=await fetch('/docflow/api/backup/restore/local/'+encodeURIComponent(n),{
...
async function restaurarBackup(){
  ...
  try{
    ...
    const resp=await fetch('/docflow/api/backup/restore',{method:'POST',headers:{Authorization:`Bearer ${token}`},body:form});
    const j=await resp.json();
    if(!resp.ok)throw new Error(j.error||'Error');
    ok.textContent='✓ Restauración completada correctamente';
    ok.style.display='block';
        ...
        try{
          ...
          if(status&&status.updaterLog?.includes('COMPLETADA')){
            ...
            const esperarServidor=setInterval(async()=>{
              try{
                await fetch('/docflow/api/health');
...
// ─── INIT ───────────────────────────────────────────────────────────────────
(async function(){
  if(S.token&&S.usuario){showApp();return}
  // Platform SSO: token exists but no user → fetch user from /me
  if(S.token&&!S.usuario){
    try{
      const r=await fetch('/docflow/api/auth/me',{headers:{'Authorization':'Bearer '+S.token}});
      if(r.ok){
        const u=await r.json();
        S.usuario=u;
        localStorage.setItem('vd_u',JSON.stringify(u));
        showApp();
        return;
      }
    }catch(e){}
